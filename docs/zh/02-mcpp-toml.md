...
某处失败。见 [workspace §4.2](03-workspace.md)。
...
> 若标志必须穿透**共享**代码,就不该放在这里 —— 改用 [workspace](03-workspace.md) member 或
...
| 两个产品差异在它们**共享**的代码里 | 拆成 [workspace](03-workspace.md) member,各自 `[build]` 标志,共享一个 `lib` |
...
> HLSL、OpenCL C 与 Metal,完整清单见 [32 — 异构硬件构建](32-heterogeneous-builds.md) —— 引擎
...
[05-build-mcpp.md](05-build-mcpp.md)。
### C++ 运行时契约(`cxx_runtime`)
...
见 [32 — 异构硬件构建](32-heterogeneous-builds.md)。
...
已移入 [04 —— Feature 与能力](04-features-and-capabilities.md),
...
已移入 [05 —— build.mcpp](05-build-mcpp.md)。
...
见 [05 — build.mcpp](05-build-mcpp.md)。把这类文件写进 `[build].ldflags` 也「能用」,
但 ldflags 是链接命令里的一串字符:没有任何东西跟踪它,改了它得到的是
`ninja: no work to do`。
### 2.16 `[hooks]` —— 项目构建生命周期命令

已移入 [06 —— 按场景选命令](06-commands-by-scenario.md)。
