...
| **精确三元组** | 某个具体目标(同时承载 `toolchain` / `linkage` / `sysroot` / `runner`,见 [02 §2.7.3](02-mcpp-toml.md)) | `[target.x86_64-linux-musl]` |
...
- **可用键**:`dependencies` / `dev-dependencies` / `build-dependencies` /
  `feature-deps.<feature>`(mcpp 2026.8.6.2+ —— 见 [05 —— build.mcpp](05-build-mcpp.md);feature 本身无条件注册,
...
参见[30 —— 裸机与 freestanding 目标](30-baremetal.md)。
