...
走的是同一条解析 —— 只有该族一个都没装时,才取生态自己的 pin。这个 pin 取自载荷正是
要求所指的那个载荷的行:`llvm` 取 `llvm@…` 的 pin,而不取 Android NDK 或 emsdk 的 ——
它们是别的载荷里的 llvm 族编译器;`mcpp:compiler=emsdk` 或
`mcpp:compiler=android-ndk` 取它自己载荷的 pin(mcpp 2026.9.15.2+)。
...
拒绝这样的构建(reason `shared-library-cxx-runtime`,见 [50](50-machine-output.md)),
并给出两条出路:在依赖的边上写 `linkage = "static"` 静态链接它 —— 仅当依赖自己的
清单没有把它约束为共享形态时才会给出这一条;或者为共享库声明一份私有副本:
...
在这条声明下,每个这样的共享库自己链接运行时包的对象。于是每个镜像各有一份运行时
类的类型信息:共享库里抛出的标准库类异常,在程序里不会被该类捕获,而共享库自己定义
的类可以。默认值从不选择私有副本,只有这条声明会。在 Mach-O 上,C++ 运行时来自图中包的
构建里每个单元都以隐藏可见性编译,使运行时的实例化永远不与系统的 libc++ 合并;因此那里
的共享库只导出源码中标注了 `[[gnu::visibility("default")]]` 的声明。
...
- **macOS 宿主上完全没有面向 Linux 的载荷**,因此从那里够不到任何 Linux 目标。这是
  发布边界,不是引擎边界。
- manifest 里显式写的 `[toolchain]`,或 `[target.X].toolchain`,**永不被推翻**。工程
  钉了一个、随后遇到这个钉子服务不了的目标时,会被拒绝,而不是悄悄换成另一个编译器。
