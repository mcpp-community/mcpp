...
的首选位置,在 `PATH` 之前([04 §2.7.3](04-mcpp-toml.md))。两个键合起来,在 CI 宿主上供给用户态模拟器,
...
- **工具不能以加速器为条件。** 加速器在依赖图之后才解析,因此这样的工具会被声明而
  永远装不上 —— 一次「工具干脆不在」却仍然成功的构建。写了这种条件的 manifest 会被
  拒绝。
- 离产物更近的那条声明胜出,且覆盖会被报告。不满足另一方所述要求的钉子会被拒绝并
  点名两边,而不是与之并存安装。
