...
**为什么 `auto` 是关的。** 模块接口编译里 86% 是没有任何导入方会读的代码生成,
所以提前发布 BMI 值很多 —— 在 mcpp 自己身上实测:`cold` 86.7s → 35.7s、
`edit-body` 80.9s → 29.8s。但调度改错是**无声**的:少一条依赖不会让构建失败,
只是某个东西不再重建了。所以它在所有平台走完 CI 之前保持 opt-in。

**它在哪里没用。** mcpp 本来就跳过级联的地方(`touch-hub`、`edit-comment`)
没有欠着的活可以挪出关键路径,这个键买不到任何东西。见
[性能对比](../../README.zh-CN.md#性能对比)。

**具体怎么做**按编译器而定,不需要你选:gcc 用 `rename()` 发布 BMI,所以代码
