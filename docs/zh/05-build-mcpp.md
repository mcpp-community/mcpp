...
而两侧被告知同一个答案。见 [02 — mcpp.toml](02-mcpp-toml.md) 的「一个包一个版本」。
...
> `ninja: no work to do`。Windows 资源请用 [`[resources]`](02-mcpp-toml.md);
> `object` 是其余一切的出口。
...
[`host-module = true`](../02-mcpp-toml.md);本节讲的是它里面应该长什么样。
...
(见 [`host-module = true`](../02-mcpp-toml.md))。`mcpp.build.*` 是引擎自己的模块族,
...
`mcpp::dep_bin("protobuf", "protoc")` 读取(见 [05 — build.mcpp](05-build-mcpp.md))。
...
**用 `[build-dependencies]` 而不是 `[dependencies]`** —— 规则包正是 [02 §2.6.1](02-mcpp-toml.md) 描述的那种
