...
| 4 | 规则包命名空间 | ~~`mcpp.rules.*` 作为约定前缀，机制上不特殊~~ **已作废，见下** | 规则包就是普通包，特殊化它只会多一套规则 |

> **⚠️ 第 4 条定案已于 2026-08-29 撤回。** 它从写下的那一刻起就不可实施：模块名当时取自
> 依赖的裸 `package.name`，而 SPEC-001 规定 `name` 是单一原子段、禁止含 `.`，所以
> `import mcpp.rules.protobuf;` 根本写不出来。实现时事实上放弃了它（唯一的规则包叫
> `grpcgen`，落在默认命名空间 `mcpplibs`），而本文档没有回写这一笔——**上一版里唯一一条
> 纯约定，是唯一一条被无声推翻的**。
>
> 替代为 **`mcpp.*` 作为模块名前缀**（不是包名前缀，这个区别正是原定案落不了地的原因），
> 保留给 mcpp 项目维护的规则，违反给警告而非硬错误——引擎判定不了谁是官方。
> 同时模块名改为由规则源码声明。见
