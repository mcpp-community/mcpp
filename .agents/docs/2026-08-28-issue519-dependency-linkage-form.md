...
⚠️ **分批表是计划,这一节是发生了什么。** 实际做成了**一个 PR**
(`mcpp#521`,2026.8.28.2):A / B1 / C / D / E / G / H 全部在内,加上一条计划里
没有的 —— **`mcpp pack` 不收合成的共享库**(§13.5,由判据 12 挖出来)。

| 批 | 落地 |
|---|---|
| A / B1 / C / D / E / G / H | ✅ `mcpp#521` |
| **pack 修复** | ✅ 同一个 PR。计划外,由判据 12 发现;与 PIC 同一条理由(轴让既有缺陷变可达) |
| **B2** | ⏸ 开成 `mcpplibs/mcpp-index#270`,等索引下限 |
| **F** | ⏸ **主动暂缓**(§11.2:一个包同时发两种形态今天没有实证需求) |

⭐ 一个 PR 而不是九个,是因为其中五项彼此耦合到「分开合入会留下半截状态」的程度:
默认 static 的零 diff 保证要 C+E 同时在,PIC 的缓存键要 C+D 同时在,而 A 的
第二段谓词要能解释 C 造出来的形状。剩下的 B1/G/H 独立,但没有理由单独排队。
