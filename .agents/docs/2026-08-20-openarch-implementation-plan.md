...
方案第 4–6 节的安排在实施中被三条实测修改,记录如下,因为每一条都推翻了写
方案时的一个假设。
...
这正是方案第 2 节所引 README 预言的失败点,而它以「接口欠定义」而非「接口
错误」的形式出现。
...
1. **一个文件只要被任何 feature 命名,就归该 feature 独占。** feature 未激活
   时它不被编译,**即便 `cfg` 块显式列出了它**;而库构建仍然报告成功,缺失
   只在消费方链接时以 `undefined reference` 出现。
2. 改为两个编码器都无条件编入之后,外来那个往 riscv64 镜像里放了 **450 字节**
   (`nm --size-sort` 实测),而整个探针的 `.text` 约 2000 字节。
...
`mcpp::xpkg_dir` 解析的是**工程声明过**的包,而不是任何已安装的包。实测:载荷
