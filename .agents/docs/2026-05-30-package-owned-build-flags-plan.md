...
- [ ] 依赖包描述中的 `mcpp.cflags` / `mcpp.cxxflags` 能作用到该包自己的 compile units。
- [ ] 包级 include dirs 在编译该包源码时优先于全局 include dirs。
- [ ] build fingerprint / cache key 包含包级 flags，避免 flags 变化后误用缓存。
- [ ] 保持现有项目级 flags 兼容，不破坏已有 manifest。
- [ ] 用 xlings 当前项目验证 `mcpp build` 和 `mcpp build --target x86_64-linux-musl`。
...
- [ ] failing test commit。
- [ ] package-owned flags implementation commit。
- [ ] xlings local validation commit / PR update。
- [ ] PR draft 创建。
- [ ] CI 每 120s 检查一次直到完成。
