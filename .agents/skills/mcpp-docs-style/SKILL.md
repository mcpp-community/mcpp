...
本规范回答五个问题:**这份文档的归属**(第一节)、**怎么写**(第二至六节)、
**它必须对得上什么**(第七节)、**它欠多少覆盖**(第九节)、
**怎么评审它**(第十三节)。
...
**边界写成事实,不写成论证。** 「当前边界」一节(第九节要求它必须存在)是一份
清单,不是一段说理:
...
「学术」在这里是具体的三件事,不是气质:每个断言的强度与它的证据相符(第六节)、
每个可粘贴的东西都可复现(第七节)、每个枚举都有分母(第九节)。
...
**规则:每份文档在开头点明它是哪一级,并且只链接相邻级。** 参考文档向上链到
教程、向下链到规范;它**不直接把读者丢进设计记录**,那是跨两级 —— 也正是第一节
那条被禁止的边。
...
- 顶部一段引言说明**这份文档回答什么问题**、它处在第八节的哪一级,以及相关
  文档的链接(用「相关文档:」,不用「姊妹篇」)。
...
| **梯度** | 它是第八节五级里的哪一级?它链接的是不是相邻级?跨级链接一律是缺陷 |
| **渐进性** | 一个从零开始的读者,能不能不跳级地到达这里 —— 前置的最小可跑形态存在吗 |
| **直观** | 只读前 15 行,能不能答出「这章讲什么、我要不要读」 |
| **覆盖度** | 分母是什么(第九节)?「当前边界」一节在不在,且是事实清单不是说理 |
