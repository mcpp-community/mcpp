...
#
# THE FIRST ONE IS THE SHAPE THIS RELEASE KEEPS PAYING FOR. `xim:glibc` is
...
#
# AND THE SECOND ONE IS ITS MIRROR. A graph-supplied C library that still
...
build_and_read() {   # extra args… → sets $report and $ldflags
    ...
    #
    # Measured on windows-2022: relation one printed both its `ok` lines and
    ...
    # nobody looked.
    #
    # AND THE SHAPE IS NARROWER THAN IT LOOKS — measured, not reasoned. A
...
#
# Measured on windows-2022: `toolchain list --format json` reported
...
for tc in gcc llvm; do
    ...
    #
    # THE FIRST VERSION HARDCODED `registry/data/xpkgs` AND WAS WRONG ON
    ...
    if [ -z "$want" ]; then
        ...
        #
        # THE FIRST VERSION SKIPPED HERE, and on windows-2022 both toolchains
        # took that branch — so relation one had no coverage on that host at all
        # and the test did not reach its conclusion.
        # THE FLAG AND ITS PATH ARE JOINED FIRST, AND BOTH SPELLINGS EXIST.
        #
