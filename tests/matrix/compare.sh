...
# 文档,而是一次测量与一份声明的比对。
#
# ⚠️⚠️ 「跳过」必须是期望表说的,不是运行时发现的。一格因为「今天这台机器没装某个
...
expected="${2:?}"
host="${3:?}"
# ⚠️ MODE 也是参数,不只是键的一列。一次扫描只产出一种体系,而拿它去比整张表,
# 另一种体系的每一行都会被报成「期望表说有而扫描没跑到」—— 一条真判据用错了论域,
# 得到的是一整屏假红。
mode="${4:?}"

# ⚠️ 键含 `mode`。两种体系(payload / graph)跑的是同一组 (host, target, compiler),
...
# ⚠️ 先断言扫描真的跑了。一格没跑与全部通过,在退出码上没有区别。
