...
| 场景 | 改了什么 | `mcpp` | `mcpp +优化` | `mcpp (旧版)` | `cmake` | `xmake` |
|---|---|---|---|---|---|---|
...
<sub>`mcpp` = mcpp@2026.8.13.1,被测的这一版 · `mcpp +优化` = **和 `mcpp` 同一个二进制**,开了 opt-in 的 `[build] bmi_schedule = "on"`(默认关闭) · `mcpp (旧版)` = mcpp@2026.8.11.3,上一个已发布版。<br>
