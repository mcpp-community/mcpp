...
import mcpp.platform.axis;    // TargetPlatform::for_lint_of — all-OS reads (#487)
...
public:
    ...
    struct ShortNameMatch {
        ...
        // How many versions a suggestion line shows (#487). The full list
        // stays one `xpkg parse` away; the hint only has to prove the
        // package is alive.
        static constexpr std::size_t kVersionsShown = 3;

        // " (1.0.0, 0.9.0, ...)" — empty when nothing is published/readable.
        // The ASCII ellipsis marks truncation so the line cannot be read as
        // "these three are all of them".
        std::string versions_label() const {
            if (versions.empty()) return {};
            std::string joined;
            auto keep = std::min(versions.size(), kVersionsShown);
            for (std::size_t i = 0; i < keep; ++i) {
                if (i) joined += ", ";
                joined += versions[i];
            }
            if (keep < versions.size()) joined += ", ...";
            return " (" + joined + ")";
        }
    };
...
Fetcher::versions_for_hit(const SearchHit& hit) const {
    ...
    for (auto* platform : {"linux", "macosx", "windows"}) {
        perPlatform.push_back(mcpp::manifest::list_xpkg_versions(
            *lua, mcpp::platform::TargetPlatform::for_lint_of(platform)));
    }
    return mcpp::manifest::merge_xpkg_versions_desc(perPlatform);
}
        ...
        for (auto& letterDir : std::filesystem::directory_iterator(pkgsDir, ec)) {
            ...
            for (auto& entry :
                ...
                for (auto* platform : {"linux", "macosx", "windows"}) {
                    perPlatform.push_back(mcpp::manifest::list_xpkg_versions(
                        content,
                        mcpp::platform::TargetPlatform::for_lint_of(platform)));
                }
                hits.push_back({id.ns + "." + id.name,
                                mcpp::manifest::merge_xpkg_versions_desc(perPlatform)});
            }
        }
    }
