...
//  lives in the rule's command string, not here)
    ...
    std::string stagedOrderOnly;
    {
        ...
        if (!staged.empty()) {
            // mcpp#405. A restored BMI carries its own imports, and `std` is
            // the one every index package has. When the CONSUMER does not
            // `import std` itself, no compile edge below names
            // `gcm.cache/std.gcm`, so the stage edge emitted above it has zero
            // consumers and ninja never runs it — the restored BMI then reads
            // `std.gcm: No such file or directory` / `Bad import dependency`,
            // an error that points nowhere near the cache.
            //
            // Cache MISS hides it: the dependency compiles locally, which puts
            // the std edge in the graph as a real prerequisite. So the first
            // project to build a package always works and every later one
            // fails, which reads as an upgrade regression.
            //
            // This aggregate is where it belongs: it exists precisely because
            // replacing compile edges with stage edges drops the ordering the
            // compile edges carried (see ORDERING above). The std BMI is that
            // same defect, one edge earlier — it was simply emitted before the
            // loop that fills `staged`.
            if (has_std_artifacts) staged.push_back(escape_ninja_path(std_bmi_dst));
            if (has_std_compat)    staged.push_back(escape_ninja_path(compat_bmi_dst));
    ...
    // package's own TUs (docs/30-build-mcpp.md), so a generated header is
    ...
    for (auto& lu : plan.linkUnits) {
        ...
        // `runtime_search_dir` either — see `linkOrderOnly`, above. The
        // deployed DLLs (`orderOnly`) and the `prepare` phonies share the one
        // order-only list of the edge.
        if (lu.kind != LinkUnit::StaticLibrary) orderOnly += linkOrderOnly;
...
std::expected<BuildResult, BuildError> NinjaBackend::build(const BuildPlan& plan,
    ...
    if (ok) {
        ...
        // and recorded in resolution.json.
        //
        // WARN-FIRST, on purpose, matching how the closure rules were rolled
        // out: the measurement behind the rule comes from one NVIDIA/X11/
        // x86_64 machine, and turning a gate hard on unverified hardware
        // stops builds that were fine. Collect real deviations first.
        for (auto const& finding :
             mcpp::build::runtime_validation::check_and_record_loader_tags(
                 plan, runtimeBefore)) {
            if (finding.status != mcpp::build::loader::TagFinding::Status::Violation)
                continue;
