...
export int run_tests(std::span<const std::string> passthrough,
    ...
    // 测试命令和 IDE configure 共用同一发现规则，避免 CDB 与真实测试
    // 编译在 member 选择、名称和 `[build].flags` 上发生漂移。
    auto discovery = discover_test_targets(*root, overrides.package_filter);
    if (!discovery) { mcpp::ui::error(discovery.error()); return 2; }
    auto testRoot = std::move(discovery->root);
    auto testTargets = std::move(discovery->targets);
